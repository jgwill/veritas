...
const initializeLocalStorage = () => {
    if (!localStorage.getItem(LOCAL_STORAGE_KEY)) {
        ...
        saveModelsToStorage([
            JSON.parse(JSON.stringify(performanceReviewModelData)),
            JSON.parse(JSON.stringify(habitatModelData))
        ]);
    }
};
...
export const getAvailableModels = async (): Promise<ModelSummary[]> => {
    ...
    return models.map(model => ({
        ...
        description: model.Note || (model.DigitalThinkingModelType === 1 
...
export const getModel = async (modelId: ModelId): Promise<DigitalModel> => {
  console.log(`Fetching model from storage: ${modelId}`);
  ...
  if (!rawModel) {
      throw new Error(`Model with ID "${modelId}" not found in local storage.`);
  }
...
export const createModel = async (config: { topic: string; type: number; elements: { name: string; description: string }[] }): Promise<DigitalModel> => {
    await new Promise(resolve => setTimeout(resolve, 50));
