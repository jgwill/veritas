    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
        ...
        <ol className="space-y-2">
          {rankedElements.map((el) => (
            <li
              key={el.Idug}
              className="flex items-center justify-between text-sm p-2 rounded-md even:bg-gray-50 dark:even:bg-gray-700/50"
            >
              <span className="font-medium text-gray-700 dark:text-gray-300">
                {rankedElements.indexOf(el) + 1}. {el.DisplayName}
              </span>
              <span className="font-bold text-tandt-primary bg-blue-100 dark:bg-blue-900/50 dark:text-blue-300 rounded-full px-2.5 py-0.5">
                {el.DominanceFactor}
              </span>
            </li>
          ))}
...
const PerformanceDashboard: React.FC<{ model: DigitalModel }> = ({ model }) => {
  ...
  const getPerformanceItemStyle = (element: DigitalElement) => {
    ...
    if (isUnacceptable && trend === -1)
      return "bg-red-100 dark:bg-red-900/40 border-l-4 border-red-500 dark:border-red-600" // Critical
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-dashed border-purple-400 dark:border-purple-600">
      ...
      {actionSuggestions && (
        <div className="space-y-4">
          {actionSuggestions.map((item, itemIndex) => (
            <div key={itemIndex} className="border-l-4 border-purple-300 dark:border-purple-500 pl-4">
          ...
          {Object.entries(tiers).map(
            ([key, tierData]) =>
              tierData.elements.length > 0 && (
                <div key={key}>
                  <h4
                    className={`text-sm font-bold uppercase tracking-wider ${tierData.color}`}
                  >{`Tier ${key}: ${tierData.label}`}</h4>
                  <ul className="mt-1 list-none space-y-1 text-sm text-gray-700 dark:text-gray-300">
                    {tierData.elements.map((el) => (
                      <li key={el.Idug} className="pl-2 border-l-2 border-gray-300 dark:border-gray-600">
                        {el.DisplayName}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Items are prioritized by status (Unacceptable first) and trend (Declining first).
              </p>
            </div>
            {hasCriticalItems && (
              <button
          ...
          <div className="space-y-3">
            {sortedElements.map((el) => (
              <div
                key={el.Idug}
                className={`shadow-sm rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between transition-colors duration-300 ${getPerformanceItemStyle(el)}`}
              >
...
const StructuringView: React.FC<StructuringViewProps> = ({ model }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {isDecisionModel ? <DecisionDashboard model={model} /> : <PerformanceDashboard model={model} />}
    </div>
  )
}
